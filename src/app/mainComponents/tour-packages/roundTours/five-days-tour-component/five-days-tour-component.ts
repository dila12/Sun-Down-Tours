...
export class FiveDaysTourComponent implements OnInit {
  selectedTours: any[] = [];
  userCountry = 'US';
  price = 0;
  ...
  get tourForDetails(): TourDetails {
    const t = this.tours.detail('tour5')!;
    return {
      title: t.title,
  ...
  get filecode(): string {
    return (
      this.tours.detail('tour5')?.filecode ??
      this.tours.meta('tour5')?.filecode ??
      '5-day-sri-lanka-tour'
    );
  }

  get bookingTour() {
    return this.tours.detail('tour5');
  }
  ...
  async ngOnInit() {
    ...
    if (isPlatformBrowser(this.platformId)) {
      ...
      this.selectedTours = await this.loadRelatedWithPrices('tour5');
      this.cdr.markForCheck();
} else {
      this.userCountry = 'US';
      this.price = 0;
      this.selectedTours = this.tours.related('tour5', 3);
      this.cdr.markForCheck();
    }
  }
