...
const supportedLangSet = new Set<string>(SUPPORTED_LANG_CODES);

/** Only match URL segments that are real language prefixes (not SEO paths like `about-us`). */
export const langCanMatch: CanMatchFn = (_route, segments) =>
  segments.length > 0 && supportedLangSet.has(segments[0].path);
...
/**
 * Shared page components under both:
 *   /about-us          (English — no prefix)
 *   /:lang/about-us    (de | it | fr | es | ru | pl | zh-CN)
 *
 * Components are never duplicated; Google Translate handles visible language.
 */
